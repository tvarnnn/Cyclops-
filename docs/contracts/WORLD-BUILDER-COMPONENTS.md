...
| `scale-unavailable` | No camera of the solve has a metric scale ratio (the depth stage failed or produced no physical fit). Metric scale is a hard dependency like the masks: without its scale tests the rule left 947 misplaced keyframes attached over the run's 24 world-arms, against 133 with them. Nothing is attached; every piece outside the anchor carries this reason and no other | forced in tests; no walk yet |
