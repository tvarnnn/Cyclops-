...
| Has capture ended but work continues (`.finalizing`) | **NOT APPLICABLE**, with a substitute | Tower **cannot observe this**. While a build runs, the files on disk are byte-identical to "stopped and never built" and to "stopped and the build crashed": the build writes nothing until it finishes, emits no event, and the writer lock is already released. A state named `finalizing` would assert what cannot be seen. Instead: `state: "stopped_unbuilt"` means *the stored figures are not the final figures*, which is the half of `.finalizing` that is true, and `build_in_progress` is **`null`** — not `false`, which would claim no build is running. Map `.finalizing` from it if you wish; the caveat travels with it. **Amended 2026-09-10:** the Tower CAN observe a live build now -- `stop_session(hold_lock=True)` holds the writer lock through the final solve and the final build -- so `lifecycle.state: "finalizing"` with `build_in_progress: true` is a real observation. `stopped_unbuilt` keeps the old caveat, and projects to `finalizing` only when the figures say a build left something drawable -- `geometry.element_count > 0` or `trajectory.pose_count > 0`, the test iOS `WorldEvidence.hasGeometry` makes -- not on `geometry.available`, which is true for a build that solved nothing; with neither count above zero there is nothing to wait for and it projects to `interrupted` |
...
| `mappingSeconds` on the Tower's clock | **IMPLEMENTED NOW** | `mapping_seconds` with `mapping_clock: "tower"`. Not clamped: when the Tower's wall clock steps backwards mid-session the field is **`null`** with `mapping_seconds_unavailable_reason` saying so, because clamping would turn an impossible value into a plausible one (five minutes of mapping reported as zero) — the `nil ≠ 0` rule again |
